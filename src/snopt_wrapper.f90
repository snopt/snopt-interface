!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
! File  snopt_wrapper.f90 --- C/C++ wrapper for SNOPT7
!
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
module snopt_wrapper
  ...
  external :: &
       sninit, snspec, sninitf, snspecf, snendf, &
  ...
  public  :: &
       f_sninit, f_sninitf, f_sninitx, f_snspec, f_snspecf, &
       f_snsetprint, &
  ...
  interface
     ...
     subroutine snLog &
         (iAbort, KTcond, MjrPrt, minimz,                             &
          n, nb, nnCon0, nnObj, nS, itn, nMajor, nMinor, nSwap,       &
          condZHZ, iObj, sclObj, ObjAdd, fObj, fMerit, penParm, step, &
          primalInf, dualInf, maxVi, maxViRel, hs,                    &
          ne, nlocJ, locJ, indJ, Jcol,                                &
          Ascale, bl, bu, Fx, fCon, yCon, x,                          &
          cu, lencu, iu, leniu, ru, lenru,                            &
          cw, lencw, iw, leniw, rw, lenrw)
       logical, intent(in) :: &
            KTcond(2)
       integer, intent(in) :: &
            iObj, lencu, lencw, leniu, leniw, lenru, lenrw, &
            MjrPrt, minimz, n, ne, nb, nlocJ, nnCon0,       &
            nnObj, nS, itn, nMajor, nMinor, nSwap,          &
            hs(nb), locJ(nlocJ), indJ(ne)
       double precision, intent(in) :: &
            condZHZ, sclObj, ObjAdd, fObj, fMerit, maxViRel, maxVi, &
            penParm(4), step, primalInf, dualInf,                   &
            Ascale(nb), bl(nb), bu(nb), fCon(nnCon0), Fx(nnCon0),   &
     ...
     subroutine snSTOP &
         (iAbort, KTcond, MjrPrt, minimz,                         &
          ...
          condHz, iObj, sclObj, ObjAdd, fObj, fMrt, PenNrm, step, &
          prInf, duInf, vimax, virel, hs,                         &
          ne, nlocJ, locJ, indJ, Jcol, negCon,                    &
          Ascale, bl, bu, Fx, fCon, gCon, gObj,                   &
          yCon, pi, rc, rg, x,                                    &
          cu, lencu, iu, leniu, ru, lenru,                        &
          cw, lencw, iw, leniw, rw, lenrw)
       logical, intent(in) :: &
       ...
       integer, intent(in) :: &
            ...
            hs(nb), locJ(nlocJ), indJ(ne)
       double precision, intent(in) :: &
            condHz, sclObj, ObjAdd, fObj, fMrt, PenNrm(4), virel, vimax, step,  &
            prInf, duInf, Ascale(nb), bl(nb), bu(nb), Fx(nnCon0), fCon(nnCon0), &
     ...
     ! SNOPTA:
     subroutine iusrfunA &
          (Status, n, x, needf, nF, F, needG, lenG, G, &
           cu, lencu, iu, leniu, ru, lenru)
       integer,          intent(in) :: &
            n, nF, needF, needG, lenG, &
            lencu, leniu, lenru
     ...
     ! SNOPTB:
     subroutine ifuncon &
         (mode, nnCon, nnJac, negCon, &
          x, fCon, gCon, Status, &
          cu, lencu, iu, leniu, ru, lenru)
       integer,          intent(in)    :: &
            mode, nnCon, nnJac, negCon, lencu, leniu, lenru
     ...
     subroutine iusrfunC &
       ...
       double precision, intent(out)   :: &
            fObj, fCon(nnCon), gObj(nnObj), gCon(negCon)
...
contains
  ...
  subroutine f_sninit &
    ...
    !===========================================================================
    ! Call snInit.  Pass provided file unit numbers.
    !===========================================================================
  ...
  subroutine f_sninitf &
       (name, len, summOn, iw, leniw, rw, lenrw) &
       bind(C,name="f_sninitf")
    ...
    !===========================================================================
    ! Call snInitF.  If a name is provided, use it as the print file name.
  ...
  end subroutine f_sninitf

  !+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

  subroutine f_sninitx &
       (printfile, plen, summfile, slen, iw, leniw, rw, lenrw) &
       bind(C,name="f_sninitx")

    integer(c_int),    intent(in), value :: plen, slen, leniw, lenrw
    character(c_char), intent(in)        :: printfile(plen), summfile(slen)
    integer(c_int),    intent(inout)     :: iw(leniw)
    real(c_double),    intent(inout)     :: rw(lenrw)
    ...
    sfile  = ''
    if (slen > 0) then
       if (summfile(1) /= c_null_char) then
          do j = 1, slen
             if (summfile(j) == c_null_char) exit
             sfile(j:j) = summfile(j)
          end do
       end if
    end if

    call snInitF &
         (pfile, sfile, iPrt, iSum, cw, lencw, iw, leniw, rw, lenrw)

  end subroutine f_sninitx

  !+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

  subroutine f_snsetprint &
  ...
  subroutine f_snsetprintf &
       (name, len, iw, leniw, rw, lenrw) &
       bind(C,name="f_snsetprintf")
  ...
  subroutine f_snspecf &
       (name, len, inform, iw, leniw, rw, lenrw) &
       bind(C,name="f_snspecf")
    ...
    ! If we have a file, try to read it.
    if (spcfile /= '') then
       call snSpecF(trim(spcfile), inform, cw, lencw, iw, leniw, rw, lenrw)
    end if

  end subroutine f_snspecf

  !+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

  subroutine f_snmema &
      (inform, nF, n, lenA, lenG, miniw, minrw, &
  ...
  subroutine f_snjac &
      (inform, nF, n, usrfunC, x, xlow, xupp, &
    ...
    integer(c_int), intent(in), value :: &
         n, nF, lenA, lenG, leniu, lenru, leniw, lenrw
    ...
    integer(c_int), intent(inout)     :: &
         ...
         iw(leniw), iu(leniu)
    real(c_double), intent(inout)     :: &
         x(n), A(lenA), rw(lenrw), ru(lenru)
    integer(c_int), intent(out)       :: &
         inform, neA, neG, miniw, minrw
  ...
  subroutine f_snopta &
      (Start, Prob, nF, n, ObjAdd, ObjRow, c_usrfun, &
    ...
    integer(c_int), intent(in), value :: &
         Start, n, nF, ObjRow, neA, neG, &
         leniu, lenru, leniw, lenrw
    real(c_double), intent(in), value :: ObjAdd

    integer(c_int),    intent(in) :: &
         iAfun(neA), jAvar(neA), iGfun(neG), jGvar(neG)
    real(c_double),    intent(in) :: &
         Flow(nF), Fupp(nF), xlow(n), xupp(n), A(neA)
    character(c_char), intent(in) :: &
         Prob(*)

    integer(c_int), intent(inout) :: &
         nInf, nS, xstate(n), Fstate(nF)
    real(c_double), intent(inout) :: &
         sInf, F(nF), Fmul(nF), x(n), xmul(n)
  ...
  subroutine f_snkera &
    ...
    integer(c_int), intent(in), value :: &
         Start, n, nF, ObjRow, neA, neG, leniu, lenru, leniw, lenrw
    real(c_double), intent(in), value :: ObjAdd
  ...
  subroutine f_snmem &
      (info, m, n, ne, negCon, nnCon, nnObj, nnJac, &
       miniw, minrw, iw, leniw, rw, lenrw) &
       bind(C,name="f_snmem")

    integer(c_int), intent(in), value :: &
         m, n, ne, negCon, nnCon, nnObj, nnJac, leniw, lenrw
    integer(c_int), intent(inout)     :: iw(leniw)
    real(c_double), intent(inout)     :: rw(lenrw)
    integer(c_int), intent(out)       :: INFO, miniw, minrw
    ...
    call snMem &
        (INFO, m, n, ne, negCon, nnCon, nnJac, nnObj, &
         mincw, miniw, minrw, &
         cw, lencw, iw, leniw, rw, lenrw)
  ...
  subroutine f_snoptb &
      (Start, Prob, m, n, ne, nnCon, nnObj, nnJac, &
    ...
    integer(c_int), intent(in), value :: &
         Start, m, n, ne, nnCon, nnObj, nnJac, iObj, &
         leniu, lenru, leniw, lenrw
    real(c_double), intent(in), value :: ObjAdd
    ...
    integer(c_int), intent(inout) :: &
         nInf, nS, indJ(ne), locJ(n+1), hs(n+m)
  ...
  subroutine f_npopt &
    ...
    integer(c_int), intent(in), value :: &
         n, nclin, ncnln, ldA, ldcJ, ldH, leniw, lenrw
    real(c_double), intent(in)    ::  &
         bl(n+nclin+ncnln), bu(n+nclin+ncnln), A(ldA,*)

    integer(c_int), intent(inout) :: iState(n+nclin+ncnln)
    real(c_double), intent(inout) :: &
         cCon(*), grad(n), Hess(ldH,*), x(n), &
  ...
  subroutine f_npkern &
      (n, nclin, ncnln, ldA, ldcJ, ldH, A, bl, bu,      &
  ...
  subroutine f_snend &
       (iw, leniw, rw, lenrw) bind(C,name="f_snend")
